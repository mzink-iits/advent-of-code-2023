...
namespace aoc.Console.Puzzles
{
    ...
    {
        private const string _input = @"eight9fhstbssrplmdlncmmqqnklb39ninejz
three656
...
";
        ...
        public static int GetCalibrationValue(string? input = null)
        {
            var lines = (input ?? _input).Split(new[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
            return lines.Sum(GetLineValues);
        }

        public static int GetLineValues(string line)
        {
            var numbers = GetNumbersWithIndex(line);
            var result = int.Parse($"{numbers.First()}{numbers.Last()}");
            System.Console.WriteLine($"{line} - {result}");
            return result;
        }

        public static int[] GetNumbersWithIndex(string line)
        {
            return _numbers.SelectMany(number => line.Contains(number) ? GetItems(line, number) : [])
        ...
        private static IEnumerable<NumberPosition> GetItems(string line, string number)
        {
            return line.AllIndexesOf(number)?.Select(index => new NumberPosition(number, index)) ?? [];
        }

        public class NumberPosition
        {
            public NumberPosition(string number, int position)
            {
                Position = position;
                ...
                {
                    Number = Array.IndexOf(_numbers, number) + 1;
                }
            }
