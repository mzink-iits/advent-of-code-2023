...
";
        ...
        public static int GetIdSumResult(string? input, int red, int blue, int green)
        {
            return (input ?? _input).Split(new[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries)
                .Select(LoadData)
                .Where(game => game.Draws.All(draw => draw.IsValid(red, blue, green)))
                .Sum(game => game.Id);
        }

        public static int GetPowers(string? input = null)
        {
            return (input ?? _input).Split(new[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries)
                .Select(LoadData)
                .Sum(game => game.GetPower());
        }

        public static Game LoadData(string line)
        {
            var gameIdMatch = GetGameId().Match(line);
            var id = int.Parse(gameIdMatch.Groups["id"].Value);

            var gamesString = GetGameId().Replace(line, string.Empty);
            var data = new Game { Id = id };
            foreach (var game in gamesString.Split(';'))
            {
                var balls = GetItems(game);
                data.Draws.Add(new Draw
                {
                    Red = balls["red"],
                    Green = balls["green"],
                    Blue = balls["blue"],
        ...
        public static Dictionary<string, int> GetItems(string data)
        {
            var draws = new Dictionary<string, int>();
            if (!string.IsNullOrWhiteSpace(data))
            {
                var balls = data.Split(",").Select(b => b.Trim());
                draws = balls.Select(b =>
                {
                    var parts = b.Split(" ");
            ...
            draws.TryAdd("red", 0);
            draws.TryAdd("blue", 0);
            draws.TryAdd("green", 0);
            return draws;
        }

        [GeneratedRegex(@"Game (?<id>\d+): ")]
        private static partial Regex GetGameId();
    }
    ...
    public class Game
    {
        ...
        public int GetPower()
        {
            return Draws.Max(draw => draw.Red) * Draws.Max(draw => draw.Blue) * Draws.Max(draw => draw.Green);
        }
    }
    ...
    public class Draw
    {
        ...
        public bool IsValid(int red, int blue, int green)
        {
